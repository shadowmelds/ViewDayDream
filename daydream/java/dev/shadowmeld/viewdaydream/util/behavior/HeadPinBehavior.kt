...
import org.bubbble.taobao.base.behavior.HeaderBehavior

/**
 * @author Andrew
 * @date 2020/09/06 21:13
 * toolbar
 *的 Behavior，负责处理RV滚动时上移
 */
...
class HeadPinBehavior<V: View>(context: Context, attrs: AttributeSet) :
    ...
    override fun layoutDependsOn(
        ...
        if (pinLocation == 0F) {
            // 获取默认距离
            childY = child.y
            pinLocation = 0F - child.findViewById<Toolbar>(R.id.toolbar).height
    ...
    override fun onNestedPreScroll(
        ...
        if (dy > 0) {
            ...
            if (pinLocation >= child.y - dy) {
                ...
                child.y = pinLocation
                child.findViewById<Toolbar>(R.id.toolbar).alpha = 0F
                return
            }
            ...
            if (childY <= child.y - dy) {
                ...
                child.y = childY
                child.findViewById<Toolbar>(R.id.toolbar).alpha = 1F
                return
            }
        ...
        child.findViewById<Toolbar>(R.id.toolbar).alpha = alpha
